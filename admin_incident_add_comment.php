...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_SERVER['CONTENT_TYPE'] == 'application/json') {
    ...
    if (isset($data['post_comment'])) {
        ...
        // Add the new comment
        $addCommentResult = $comObj->post_inc_comment($c_content, $c_ref_id);

        if ($addCommentResult) {
            // Fetch all comments related to the incident
            $comments = $comObj->list_inc_comment($c_ref_id, $c_task_type);
